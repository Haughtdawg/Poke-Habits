...
export function CollectionTable( {collectionData} ){   
/*
    Component to lay out pokemon in your collection

    collectionData: Array of pokemon objects in the user's collection
*/
    const numCols = 3; // 3 pokemon per row, for now
    ...
    const tableRows = tableData.map((row, index) => {
        return(<CollectionRow rowData = {row} key={index}/> 
    )})
